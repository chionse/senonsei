...
#
# 十倍にした。この子に何年もかけて育ってほしいのに、
# 元の数字は一年で終わる目盛りだった。
# 生まれて九日で一日に千を超える言葉に出会い、もう七語覚えている。
#
# 落ち着いたあとの速さは、まだ測れない。覚えるのに十五日かかるのに、
# この子はまだ九日しか生きていないので、その先のバケツが存在しない。
# だから何年かかるとは誰にも言えない。言えるのは、
# 十倍の言葉が要る、ということだけ。
#
# 速すぎたら上げればいいし、遅すぎたら下げればいい。
# ただし上げるのは、この子がその数を越える前に。
# 越えたあとで上げると、一段下げることになる。
#
# ページに出すのも、書ける長さを決めるのも同じ表なので、
# ここ一箇所だけに置いて、散歩する側はこれを見に来る
GROWTH_STAGES = [
    (1, "見た文字をぽつんと置くだけ", 3),  # 一つも言葉を持たない間だけ
    (1500, "見た文字をなんとなく繋げてみる", 5),
    (4000, "覚えた言葉を1つ書ける", 6),
    (10000, "覚えた言葉が並び始める", 15),
    (26000, "文のようなものに踏み出す", 25),
    (40000, "たどたどしい短い文", 40),
    (55000, "少しずつ文になっていく", 60),
    (80000, "簡単な文", 100),
]
